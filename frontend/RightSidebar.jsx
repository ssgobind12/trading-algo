          ...
          {portfolioError ? (
            ...
            <div>
              ...
              <div style={{ padding: '0 16px' }}>
                <h3 style={{ fontSize: '14px', color: 'var(--text-main)', marginBottom: '12px' }}>Current Holdings</h3>
                {portfolio.holdings && portfolio.holdings.length > 0 ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {portfolio.holdings.map((h, i) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px', backgroundColor: 'rgba(255,255,255,0.02)', borderRadius: '4px' }}>
                        <div>
                          <div style={{ fontWeight: 'bold', fontSize: '13px' }}>{h.company_name}</div>
                          <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Qty: {h.quantity}</div>
                        </div>
                ...
                {tradeHistory && tradeHistory.length > 0 ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {tradeHistory.map((trade, i) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', backgroundColor: 'rgba(255,255,255,0.02)', borderRadius: '6px', borderLeft: `3px solid ${trade.side === 'BUY' ? 'var(--up-color)' : 'var(--down-color)'}` }}>
                        <div>
                          <div style={{ fontWeight: 'bold', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <span style={{ color: trade.side === 'BUY' ? 'var(--up-color)' : 'var(--down-color)' }}>{trade.side}</span>
                            {trade.symbol}
                          </div>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: '13px', fontWeight: 'bold' }}>₹{trade.price.toFixed(2)}</div>
                          <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Qty: {trade.quantity}</div>
                        </div>
                      </div>
                    ))}
                  </div>
